...
const Movie = ({ data }: any) => {
  ...
  const { data: recommendedTitlesRes, isFetched: isFetchedRecommmended } =
    useQuery(`recommended-titles-tv-${data.id}`, getRecommendedTitles, {
      refetchOnWindowFocus: false,
    });
  const { data: similarTitlesRes, isFetched: isFetchedSimilar } = useQuery(
    `similar-titles-tv-${data.id}`,
    getSimilarTitles,
    {
      refetchOnWindowFocus: false,
    }
  );
  const { data: imagesRes } = useQuery(`images-tv-${data.id}`, getImages, {
    refetchOnWindowFocus: false,
  });
  const { data: videoRes } = useQuery(`videos-tv-${data.id}`, getVideos, {
    refetchOnWindowFocus: false,
  });
  const { data: reviewsRes } = useQuery(`reviews-tv-${data.id}`, getReviews, {
    refetchOnWindowFocus: false,
  });
  const { data: creditRes } = useQuery(`credit-tv-${data.id}`, getCredit, {
    refetchOnWindowFocus: false,
  });
  ...
  const handleScrollPrev = () => {
    setTravel(300);
    if (tvImagesContainerRef.current) {
      tvImagesContainerRef.current.scroll({
        left: 0,
        behavior: "smooth",
      });
    }
        ...
        toast.custom((t) => (
          ...
          >
            <div className="">
              ...
              >
                Series added to the liked list
              </h1>
            </div>
            <div>
              <svg
          ...
          >
            <div className="">
              <h1
                className={`${jose.className} xsm:text-xs md:text-sm lg:text-lg`}
              >
                Series is already in the liked list!
              </h1>
            </div>
            <div>
              <svg
          ...
          >
            <div className="">
              <h1
                className={`${jose.className} xsm:text-xs md:text-sm lg:text-lg`}
              >
                Series is already in the watchlist!
              </h1>
            </div>
            <div>
              <svg
  ...
  return (
    <div className="bg-primary">
      ...
      <div className="grid">
        ...
        />
        <div
          className={`bg-primary bg-opacity-80 p-11 md:flex justify-center col-span-full row-span-full`}
        >
          <div className="mr-10 ">
            <Image
              // placeholder=""
          ...
          <div className="basis-1/2 text-white">
            ...
            {/* Release Date */}
            <div className="flex">
              <p className="xsm:text-xs md:text-lg mr-2">First Air Date: </p>
              <p className="xsm:text-xs md:text-lg mr-2 font-semibold">
                {readableReleaseDate}
              </p>
            </div>
            ...
            {/* Number of Seasons */}
            <div className="flex">
              <p className="xsm:text-xs md:text-lg mr-2">Number of Seasons: </p>
              <p className="xsm:text-xs md:text-lg mr-2 font-semibold">
            ...
            {/* Number of Episodes */}
            <div className="flex">
              <p className="xsm:text-xs md:text-lg mr-2">
                Number of Episodes:{" "}
              </p>
              <p className="xsm:text-xs md:text-lg mr-2 font-semibold">
            ...
            {/* Runtime */}
            <div className="flex">
              <p className="xsm:text-xs md:text-lg mr-2">Episode Runtime: </p>
              <p className="xsm:text-xs md:text-lg mr-2 font-semibold">
            ...
            {/* Adult */}
            <p className="bg-slate-300 text-slate-800 w-fit px-2 rounded-md font-bold xsm:text-xs md:text-lg">
