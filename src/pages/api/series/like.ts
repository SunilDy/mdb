...
export default async function handler(
    req: NextApiRequest,
    res: NextApiResponse<{error?: any, status?: string, message?: string, likedSeries?: string[], result?: any}>
) {
    ...
    if (req.method === 'POST') {
        try {
            ...
            if(user) {
                ...
                if(alreadyContains) {
                    res.status(403).send({status: "err", message: "Series already exists in the likedlist"})
                } else {
                    if(session?.user)
                        User.findOneAndUpdate({name: session.user.name}, {
                            ...
                            res.status(201).send({status: "ok", message: "Added to the liked list"})
                        })
                }
            } else {
...
<<<<<<< HEAD
                    let result = await axios.get(`https://api.themoviedb.org/3/tv/${user.likedSeries[id]}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`)
=======
                    let result = await axios.get(`https://api.themoviedb.org/3/tv/${user.likedSeries[id]}?NEXT_PUBLIC_API_KEY=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`)
