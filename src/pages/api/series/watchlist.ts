...
export default async function handler(
    req: NextApiRequest,
    res: NextApiResponse<{error?: any, status?: string, message?: string, watchListSeries?: string[], result?:any}>
) {
    ...
    if (req.method === 'POST') {
        try {
            ...
            if(user) {
                ...
                if(alreadyContains) {
                    res.status(403).send({status: "err", message: "Series already exists in the watchlist"})
                } else {
                    if(session?.user)
...
<<<<<<< HEAD
                    let result = await axios.get(`https://api.themoviedb.org/3/tv/${user.watchListSeries[id]}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`)
=======
                    let result = await axios.get(`https://api.themoviedb.org/3/tv/${user.watchListSeries[id]}?NEXT_PUBLIC_API_KEY=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`)
