...
const Movie = ({ data }: any) => {
  ...
  const getRecommendedTitles = () => {
    return axios.get(
      `https://api.themoviedb.org/3/movie/${data.id}/recommendations?api_key=${process.env.API_KEY}`
    );
  };

  const getSimilarTitles = () => {
    return axios.get(
      `https://api.themoviedb.org/3/movie/${data.id}/similar?api_key=${process.env.API_KEY}`
    );
  };

  const getImages = () => {
    return axios.get(
      `https://api.themoviedb.org/3/movie/${data.id}/images?api_key=${process.env.API_KEY}&include_image_language=en,null`
    );
  };

  const getVideos = () => {
    return axios.get(
      `https://api.themoviedb.org/3/movie/${data.id}/videos?api_key=${process.env.API_KEY}`
    );
  };

  const getReviews = () => {
    return axios.get(
      `https://api.themoviedb.org/3/movie/${data.id}/reviews?api_key=${process.env.API_KEY}`
    );
  };

  const getCredit = () => {
    return axios.get(
      `https://api.themoviedb.org/3/movie/${data.id}/credits?api_key=${process.env.API_KEY}`
    );
  };
